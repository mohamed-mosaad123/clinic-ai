    ...
    public class AllProfiles : Profile
    {
        ...
        public AllProfiles()
        {
            ...
            CreateMap<Appointment, AppointmentReadDto>()
                ...
                .ForMember(des => des.AppointmentDate, op => op.MapFrom(src => src.ScheduledDate));
            CreateMap<AppointmentWriteDto, Appointment>()
                .ForMember(des => des.CreatedAt, op => op.MapFrom(src => DateTime.UtcNow))
                .ForMember(des => des.ScheduledDate, op => op.MapFrom(src => src.AppointmentDate));
            ...
            CreateMap<AiAnalysis, AiAnalysisReadDto>()
                .ForMember(des => des.PatientName, op => op.MapFrom(src =>
                    src.Patient != null && src.Patient.Person != null
                        ? FullName(src.Patient.Person.FirstName, src.Patient.Person.LastName)
                        : "Unknown"))
                .ForMember(des => des.Inputs, op => op.MapFrom(src =>
                    string.IsNullOrWhiteSpace(src.InputsJson)
                        ? new Dictionary<string, object>()
                        : JsonSerializer.Deserialize<Dictionary<string, object>>(src.InputsJson, new JsonSerializerOptions()) ?? new Dictionary<string, object>()));
